...
# ── 인덱스 산출물 이름 (임베딩은 OpenAI 단일) ────────────
# 기존 산출물 이름을 유지(재인덱싱/재커밋 회피).
COLLECTION = "reports_openai"          # Chroma 컬렉션
BM25_FILE = "bm25_openai.pkl"          # BM25 인덱스
...
def bm25_path() -> Path:
    return STORAGE_DIR / BM25_FILE


def npz_path() -> Path:
    """인메모리(numpy) 벡터 저장 파일 경로."""
    return STORAGE_DIR / f"{COLLECTION}.npz"
