...
def _strip_pageno(heading: str) -> str:
    """제목 끝에 딸려온 단독 쪽번호 제거 ('…투자환경 10' → '…투자환경')."""
    return re.sub(r"\s+\d{1,3}$", "", heading).strip()
...
RE_KW = re.compile(r"^[◈※]?\s*(요약|목차|시사점|출처|참고\s*문헌|현지\s*인터뷰)\s*$")
...
def _table_to_md(rows: list) -> str:
    ...
    # 전부 비어 있는 열 제거 (추출 누락된 행라벨 열 등 → '|  |' 흔적 방지)
...
def parse_and_chunk(pdf_path: str, meta: dict, doc_id: str) -> list[dict]:
    ...
    def new_chunk(text, page, ctype, **extra):
        ...
        if not chapter:
            return
        # 본문 파편(아주 짧은 청크)은 검색 가치가 낮아 제외 (표/요약 등은 유지)
        if ctype == "body" and len(text) < 25:
            return
        c = {
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for pno, page in enumerate(pdf.pages, start=1):
            ...
            for ln in lines:
                ...
                if m_ch:
                    flush()
                    chapter = _strip_pageno(f"{m_ch.group(1)}. {m_ch.group(2)}")
                    chapter_type = _chapter_type(chapter)
                    section = subsection = ""
                    continue
                if m_se:
                    flush()
                    section = _strip_pageno(ln)
                    subsection = ""
                    continue
                if m_su:
                    flush()
