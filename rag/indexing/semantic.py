...
def _create_ctx(client, base):
    """추론강도(low) 우선 호출, 미지원 모델이면 해당 인자 빼고 재시도."""
    try:
        return client.chat.completions.create(reasoning_effort=config.CTX_REASONING_EFFORT, **base)
    except Exception as e:  # noqa: BLE001 — 미지원 파라미터(BadRequest/TypeError)만 폴백
        if "reasoning_effort" in str(e) or "Unsupported" in str(e) or isinstance(e, TypeError):
            return client.chat.completions.create(**base)
        raise


def _ctx_one(c: dict, api_key: str) -> str:
    """청크 1개에 검색용 맥락 1문장 생성. 429(RPM/TPM)는 백오프 후 재시도해 누락 방지."""
    ...
    base = dict(
        ...
        max_completion_tokens=config.CTX_MAX_TOKENS,   # 넉넉히(추론+출력 합산 상한)
    )
    from openai import RateLimitError
    for attempt in range(config.CTX_MAX_RETRIES):
        try:
            r = _create_ctx(client, base)
            ...
            time.sleep(min(1.0 * (attempt + 1), 8.0))   # 점증 백오프
        except Exception as e:  # noqa: BLE001 — 그 외 오류는 빈 맥락(구조헤더 폴백)
            log.warning("[contextualize] %s", e)
            return ""
    log.warning("[contextualize] 429 재시도 소진 → 빈 맥락")
    return ""


def contextualize(chunks: list[dict], api_key: str | None = None, workers: int | None = None):
    """각 청크에 c['context'] (LLM 1문장 맥락) 부여. 병렬 호출(429 회피용 동시성 제한)."""
    key = resolve_key(api_key)
    workers = workers or config.CTX_WORKERS
    done = [0]
    ...
    # 2차 보충: 남은 빈맥락만 저동시성으로 재시도(레이트리밋 여파 최소화 → 누락 0 지향)
    missing = [c for c in chunks if not c.get("context")]
    if missing:
        log.info("빈맥락 %d건 → 2차 보충 재시도(동시성 2)", len(missing))
        with ThreadPoolExecutor(max_workers=2) as ex:
            list(ex.map(lambda c: c.__setitem__("context", _ctx_one(c, key)), missing))

    n_empty = sum(1 for c in chunks if not c.get("context"))
    log.info("맥락 생성 완료: %d청크 (잔여 빈맥락 %d)", len(chunks), n_empty)
